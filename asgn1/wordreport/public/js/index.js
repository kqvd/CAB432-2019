...
function loadAnimation() {
    ...
    const message = `
                    <p>Please wait while we fetch some words..</p>
                    `;

    const displayAnimation = document.getElementById("loadingAnimation");
    const displayText = document.getElementById("loadingText");
    displayAnimation.innerHTML = animation;
...
function getCountry() {
    ...
    if (countries.includes(country)) {
        for (var i = 0; i < abbreviations.length; i++) {
            if (abbreviations[i][0] == country) {
                abbreviation = abbreviations[i][1];
            }

            // Replace value in HTML
            document.getElementById("myInput").value = abbreviation;
        ...
        alert("Sorry, we don't have results for '" + country + "'.");
        return false;
    }
} 